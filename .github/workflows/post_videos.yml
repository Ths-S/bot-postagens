name: Post Shorts & Reels

on:
  schedule:
    # Executa às 06:00, 14:00 e 22:00 todos os dias
    - cron: "0 6,14,22 * * *"
  workflow_dispatch: # permite rodar manualmente também

jobs:
  post-videos:
    runs-on: ubuntu-latest
    steps:
      - name: Checkout repositório
        uses: actions/checkout@v3

      - name: Configurar Python
        uses: actions/setup-python@v4
        with:
          python-version: "3.11"

      - name: Instalar dependências
        run: |
          python -m pip install --upgrade pip
          pip install google-auth google-auth-oauthlib google-api-python-client requests

      - name: Executar upload para YouTube
        env:
          YOUTUBE_CLIENT_SECRET_JSON: ${{ secrets.YOUTUBE_CLIENT_SECRET_JSON }}
          YOUTUBE_TOKEN_PICKLE: ${{ secrets.YOUTUBE_TOKEN_PICKLE }}
        run: |
          echo "🚀 Rodando upload para YouTube"
          python upload_youtube.py

      - name: Executar upload para Instagram
        env:
          IG_ACCESS_TOKEN: ${{ secrets.IG_ACCESS_TOKEN }}
          IG_USER_ID: ${{ secrets.IG_USER_ID }}
        run: |
          echo "🚀 Rodando upload para Instagram"
          python upload_instagram.py

      - name: Mover vídeos de pending para posted
        run: |
          echo "📂 Movendo vídeos processados"
          mkdir -p videos/posted
          for file in videos/pending/*; do
            if [ -f "$file" ]; then
              mv "$file" videos/posted/
              echo "✅ $file movido para videos/posted/"
            fi
          done
